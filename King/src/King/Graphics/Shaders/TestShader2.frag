...
uniform float ambient_strength = 0.25f;
...
vec3 calculateDirLight(Light light, vec3 normal, vec3 viewDir) {
	...
	vec3 ambient = ambient_strength * light.color * vec3(texture(ourTexture, v_TexCoord)) * material.diffuse;
	vec3 diffuse  = light.intensity  * diff * light.color * vec3(texture(ourTexture, v_TexCoord)) * material.diffuse;
...
vec3 calculatePointLight(Light light, vec3 normal, vec3 fragPos, vec3 viewDir) {
    ...
    // combine results
	vec3 ambient =	light.intensity  * light.color * vec3(texture(ourTexture, v_TexCoord)) * material.diffuse;
	vec3 diffuse  = light.intensity  * diff * light.color * vec3(texture(ourTexture, v_TexCoord)) * material.diffuse;
    vec3 specular = light.intensity * spec * light.color * material.specular;
    ambient  *= attenuation;
    diffuse  *= attenuation;
